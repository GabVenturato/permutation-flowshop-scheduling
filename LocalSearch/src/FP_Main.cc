...
int main(int argc, const char* argv[])
{
  ...
  FP_CostComponent1 cc1(in, 1, true);
  FP_CostComponent2 cc2(in, 1, true);
 
  FP_MoveDeltaCostComponent1 dcc1(in, cc1);
  FP_MoveDeltaCostComponent2 dcc2(in, cc2);

  // helpers
  FP_StateManager FP_sm(in);
  FP_MoveNeighborhoodExplorer FP_nhe(in, FP_sm);
  ...
  // runners
  HillClimbing<FP_Input, FP_State, FP_Move> FP_hc(in, FP_sm, FP_nhe, "FP_MoveHillClimbing");
  SteepestDescent<FP_Input, FP_State, FP_Move> FP_sd(in, FP_sm, FP_nhe, "FP_MoveSteepestDescent");
  SimulatedAnnealing<FP_Input, FP_State, FP_Move> FP_sa(in, FP_sm, FP_nhe, "FP_MoveSimulatedAnnealing");

  // tester
  Tester<FP_Input, FP_Output, FP_State> tester(in,FP_sm,FP_om);
  MoveTester<FP_Input, FP_Output, FP_State, FP_Move> swap_move_test(in,FP_sm,FP_om,FP_nhe, "FP_Move move", tester); 
